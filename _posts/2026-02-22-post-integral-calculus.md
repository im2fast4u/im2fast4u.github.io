...
The main part of integral calculus is finding the area under a curve. This area problem stuck as a mystery for about two millenia. Let's first look at *how* to solve it. When it comes to the area of a curvy shape, sometimes we estimate the area by making rectangles and filling the shape with them. We know how to find the area of rectangles: width times height. For example, if we were asked to find the area under the curve $2x^2 - 4$ from $x=3$ to $x=4$, we could make two rectangles and find the area of each one. Then, we could add them together.
...
*When estimating the curve, the more rectangles there are and the smaller they are, the better the approximation becomes*
...
*Note: The integration simbol* $\int$ *is a stretched S to denote that it's adding infinite rectangles. In the integration format,* $\int_{a}^{b} f(x) dx$, $f(x)$ *is the height of each rectangle which gets multiplied by the width* $dx$
