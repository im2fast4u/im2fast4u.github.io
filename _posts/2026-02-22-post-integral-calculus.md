...
The main part of integral calculus is finding the area under a curve. This area problem stuck as a mystery for a long time. Let's first look at *how* to solve it. When it came to the area of a curvy shape, people estimated the area by making rectangles and filling the shape with them. We know how to find the area of rectangles: width times height. For example, if we were asked to find the area under the curve $2x^2 - 4$ from $x=3$ to $x=4$, we could make two rectangles and find the area of each one. Then, we could add them together.
