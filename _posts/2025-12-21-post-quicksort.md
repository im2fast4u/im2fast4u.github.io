...
To sort an array with selection sort, you have to choose the smallest number in the unsorted array. You swap that number with the first number in the array. That number is in the correct spot. Then, you choose the second smallest number and replace it with the second number in your array. You keep repeating that until all the numbers are in the correct place. That is selection sort. The average time in big O notation for selection sort is O($n^2$). However, quicksort is much faster, and that's what what this whole post is about.
